...
export default function DifficultyGraph() {
  ...
  const getTierColor = (tier) => {
    switch (tier) {
      case 'TIER 1': return '#10B981'; // Vibrant Emerald
      case 'TIER 2': return '#34D399'; // Bright Mint
      case 'TIER 3': return '#EF4444'; // Crimson Bronze
      case 'TIER 4': return '#3B82F6'; // Electric Blue
      case 'TIER 5': return '#94A3B8'; // Slate
  ...
  return (
    <div className="bg-[#12161F] p-6 md:p-8 rounded-xl border border-[#252E3E] space-y-6 shadow-2xl">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-[#252E3E] pb-5">
        <div>
        ...
        <div className="flex flex-col items-start md:items-end gap-2.5">
          ...
          <div className="flex items-center space-x-1.5 bg-[#090B0E] p-1.5 rounded-lg border border-[#252E3E] font-mono text-xs">
            <button
              ...
              <button
                ...
                className={`px-2.5 py-1 rounded-md font-bold transition-all border text-[11px] ${
                  selectedTier === t
                    ...
                    : 'bg-[#090B0E] text-gray-300 border-[#252E3E] hover:border-dew-green/50'
                }`}
              >
                {t}
              </button>
      ...
      <div className="relative max-w-md">
        ...
        <input
          ...
          className="w-full bg-[#090B0E] text-white font-mono text-xs rounded-lg pl-10 pr-3 py-2.5 border border-[#252E3E] focus:outline-none focus:border-dew-green transition-all shadow-inner"
        />
      </div>

      {/* SVG Interactive Scatter Plot Graph */}
      <div className="relative bg-[#090B0E] border border-[#252E3E] rounded-lg p-4 overflow-x-auto shadow-inner">
        <svg viewBox={`0 0 ${SVG_WIDTH} ${SVG_HEIGHT}`} className="w-full h-auto select-none min-w-[700px]">
          ...
          {/* X-Axis Labels */}
          <text x={PADDING_LEFT} y={SVG_HEIGHT - 20} fill="#10B981" fontSize="11" fontFamily="monospace" fontWeight="bold">
            #1 HARDEST SHOW
          </text>
        ...
        {hoveredShow && (
          <div className="mt-4 bg-[#181E2B] border border-dew-green/40 rounded-lg p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3 shadow-2xl animate-in fade-in duration-150">
            <div className="space-y-1">
              <div className="flex items-center space-x-2">
                ...
                <span className="text-xs font-mono px-2 py-0.5 rounded bg-[#090B0E] text-dew-green border border-[#252E3E]">
                  {hoveredShow.tier}
                </span>
              </div>
            ...
            <div className="text-right font-mono bg-[#090B0E] px-4 py-2 rounded-md border border-[#252E3E]">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-5 gap-3 font-mono text-xs">
          {rawShows.slice(0, 5).map((show, idx) => (
            <div
              key={show.name}
              onClick={() => setHoveredShow(show)}
              className="bg-[#181E2B] p-3 rounded-lg border border-[#252E3E] cursor-pointer hover:border-dew-green/60 shadow-md transition-all space-y-1"
            >
