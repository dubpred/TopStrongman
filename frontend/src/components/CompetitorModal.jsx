...
export default function CompetitorModal({ athlete, onClose }) {
  ...
  return (
    ...
    >
      <div 
        className="relative w-full h-full sm:h-auto max-w-4xl sm:max-h-[90vh] flex flex-col bg-[#0B130B] border-0 sm:border border-dew-green/40 rounded-none sm:rounded-3xl shadow-2xl overflow-hidden my-auto"
        onClick={(e) => e.stopPropagation()}
      >
        
        {/* Top Header Gradient Glow */}
        <div className="h-2 w-full bg-gradient-to-r from-dew-green via-dew-yellow to-dew-red shrink-0"></div>

        {/* Sticky Header */}
        <div className="sticky top-0 z-30 bg-[#0B130B]/95 backdrop-blur-md border-b border-dew-green/20 px-4 sm:px-6 py-3.5 flex items-center justify-between shrink-0">
          <div className="flex items-center space-x-2.5 truncate pr-2">
            <span className="bg-dew-green text-black font-display text-sm sm:text-base font-black rounded-lg px-2 py-0.5 shrink-0">
              #{globalRank}
            </span>
        ...
        <div className="overflow-y-auto flex-1 p-4 sm:p-6 md:p-8 space-y-6">
          ...
          {/* Hero Section */}
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 p-5 sm:p-6 rounded-2xl bg-gradient-to-b from-[#142214] to-[#080D08] border border-dew-green/20">
            ...
            {/* Total Points Card */}
            <div className="bg-[#080D08] border border-dew-green/40 rounded-2xl px-6 py-4 text-center min-w-[150px] w-full sm:w-auto shadow-dew-glow">
          ...
          <div className="grid grid-cols-3 divide-x divide-dew-green/20 bg-[#080D08] rounded-2xl border border-dew-green/20 text-center py-3.5">
            <div className="px-1">
          ...
          <div className="space-y-3">
            ...
            <div className="block md:hidden space-y-3">
              {contributions && contributions.map((c, idx) => (
                <div 
                  key={idx} 
                  className={`bg-[#080D08] border ${idx === 0 ? 'border-dew-green/60 bg-dew-green/5' : 'border-dew-green/20'} rounded-2xl p-4 space-y-2`}
                >
                  ...
                  <div className="grid grid-cols-3 gap-2 pt-2 border-t border-dew-green/10 text-center font-mono text-xs">
                    <div className="bg-dew-card/50 rounded-lg p-1.5">
            ...
            <div className="hidden md:block dew-glass-card overflow-hidden">
              <div className="overflow-x-auto">
                <table className="w-full text-left text-xs font-mono">
                  <thead>
                    <tr className="bg-[#080D08] border-b border-dew-green/20 text-gray-400 uppercase">
                  ...
                  <tbody className="divide-y divide-dew-green/10">
                    {contributions && contributions.map((c, idx) => (
                      <tr key={idx} className={`hover:bg-dew-card/60 transition-colors ${idx === 0 ? 'bg-dew-green/5' : ''}`}>
        ...
        <div className="bg-[#080D08] border-t border-dew-green/20 px-4 sm:px-6 py-3 flex justify-between items-center shrink-0">
          <button
            onClick={() => {
              const modalBody = document.querySelector('.overflow-y-auto');
              if (modalBody) modalBody.scrollTop = 0;
            }}
            className="flex items-center space-x-1.5 text-xs font-mono text-gray-400 hover:text-dew-green transition-colors py-1"
          >
          ...
          <button
            onClick={onClose}
            className="px-4 py-2.5 rounded-xl bg-dew-green text-black font-heading font-bold text-xs hover:bg-dew-neon transition-all shadow-dew-glow active:scale-95"
          >
