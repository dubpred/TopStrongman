...
export default function CompetitorModal({ athlete, onClose }) {
  ...
  return (
    ...
    >
      <div 
        className="relative w-full h-full sm:h-auto max-w-4xl sm:max-h-[90vh] flex flex-col bg-[#12141B] border-0 sm:border border-[#1E222D] rounded-none sm:rounded-xl shadow-2xl overflow-hidden my-auto"
        onClick={(e) => e.stopPropagation()}
      >
        
        {/* Top Header Gold Accent Line */}
        <div className="h-1.5 w-full bg-gradient-to-r from-dew-green via-amber-400 to-yellow-500 shrink-0"></div>

        {/* Sticky Header */}
        <div className="sticky top-0 z-30 bg-[#12141B]/95 backdrop-blur-md border-b border-[#1E222D] px-4 sm:px-6 py-3.5 flex items-center justify-between shrink-0">
        ...
        <div className="overflow-y-auto flex-1 p-4 sm:p-6 md:p-8 space-y-6">
          ...
          {/* Hero Section */}
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 p-5 sm:p-6 rounded-lg bg-[#161920] border border-[#1E222D]">
            ...
            {/* Total Points Card */}
            <div className="bg-[#0F1115] border border-dew-green/40 rounded-lg px-6 py-3.5 text-center min-w-[150px] w-full sm:w-auto shadow-dew-glow">
          ...
          <div className="grid grid-cols-3 divide-x divide-[#1E222D] bg-[#161920] rounded-lg border border-[#1E222D] text-center py-3.5">
            <div className="px-1">
          ...
          <div className="space-y-3">
            ...
            <div className="block md:hidden space-y-3">
              {contributions && contributions.map((c, idx) => (
                <div 
                  key={idx} 
                  className={`bg-[#161920] border ${idx === 0 ? 'border-dew-green/60 bg-dew-green/5' : 'border-[#1E222D]'} rounded-lg p-4 space-y-2`}
                >
                  ...
                  <div className="grid grid-cols-3 gap-2 pt-2 border-t border-[#1E222D] text-center font-mono text-xs">
                    <div className="bg-[#0F1115] rounded-md p-1.5 border border-[#1E222D]">
            ...
            <div className="hidden md:block bg-[#161920] overflow-hidden border border-[#1E222D] rounded-lg shadow-xl">
              <div className="overflow-x-auto">
                <table className="w-full text-left text-xs font-mono">
                  <thead>
                    <tr className="bg-[#1C212C] border-b border-[#1E222D] text-gray-300 uppercase">
                  ...
                  <tbody className="divide-y divide-[#1E222D]/60">
                    {contributions && contributions.map((c, idx) => (
                      <tr key={idx} className={`hover:bg-[#1D2330] transition-colors ${idx === 0 ? 'bg-dew-green/5' : ''}`}>
        ...
        <div className="bg-[#12141B] border-t border-[#1E222D] px-4 sm:px-6 py-3 flex justify-between items-center shrink-0">
          <button
            onClick={() => {
          ...
          <button
            onClick={onClose}
            className="px-4 py-2 rounded-lg bg-dew-green text-black font-heading font-bold text-xs hover:bg-[#FBBF24] transition-all shadow-dew-glow active:scale-95"
          >
